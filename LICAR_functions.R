...
## ADD PEO class MRM transition:NHG 141 Pre-Pro=141 
  if( !(lipidClass %in% c("AcylCarnitine", "LPC", "LPCql", "LPCO", "LPCOql", "LPE", "LPENHG", "PC", "PCO", "PCP", "PE", "PEO","PENHG", 
                          "PG", "PGNHG", "PI","LPI", "PIO","PINHG", "PS","LPS", "PSNHG", "S1P", "S1Pql", "SM","PSO") ) ) {
    stop("For lipid group 'Head Group', lipid class must be 'AcylCarnitine', 'LPC', 'LPCql', 'LPCO', 'LPCOql', 'LPE', 'LPENHG', 'PC', 'PCO', 'PCP', 'PE','PEO', 'PENHG', 'PG', 
         'PGNHG', 'PI','LPI','PIO', 'PINHG', 'PS','PSO','LPS', 'PSNHG', 'S1P', 'S1Pql' or 'SM'.") } else {
           ...
           if(  ( lipidClass %in% c("AcylCarnitine") ) && ( inputData$Product <= 85.2 && inputData$Product >= 84.8 ) ) {
           ...
           }  else if(  ( lipidClass %in% c("PEO") ) && ( ( inputData$Precursor - inputData$Product ) <= 141.2 && ( inputData$Precursor - inputData$Product ) >= 140.8 ) ) {
             isoCorrect_head(inputData, constant_C = 3, constant_H = -1, constant_O = 3, constant_N = 0 ) #NEED TO CHECK IN WED WXX
             ...
             isoCorrect_head(inputData, constant_C = 3, constant_H = -3, constant_O = 4, constant_N = 0 )
           } else if(  ( lipidClass %in% c("LPI")  ) && ( inputData$Product <= 241.2 && inputData$Product >= 240.8 ) ) {
             isoCorrect_head(inputData, constant_C = 3, constant_H = 0, constant_O = 8, constant_N = 0 ) #NEED TO CHECK IN WED WXX
           } else if(  ( lipidClass %in% c("PIO")  ) && ( inputData$Product <= 241.2 && inputData$Product >= 240.8 ) ) {
             isoCorrect_head(inputData, constant_C = 3, constant_H = 0, constant_O = 4, constant_N = 0 ) #NEED TO CHECK IN WED WXX
             ...
             isoCorrect_head(inputData, constant_C = 3, constant_H = -3, constant_O = 4, constant_N = 0 )
           } else if(  ( lipidClass %in% c("LPS") ) && ( ( inputData$Precursor - inputData$Product ) <= 87.2 && ( inputData$Precursor - inputData$Product ) >= 86.8 ) ) {
             isoCorrect_head(inputData, constant_C = 3, constant_H = 0, constant_O = 7, constant_N = 0 ) #NEED TO CHECK IN WED WXX
           }else if(  ( lipidClass %in% c("PSO") ) && ( ( inputData$Precursor - inputData$Product ) <= 87.2 && ( inputData$Precursor - inputData$Product ) >= 86.8 ) ) {
             isoCorrect_head(inputData, constant_C = 3, constant_H =0, constant_O = 7, constant_N = 0 ) #NEED TO CHECK IN WED WXX
